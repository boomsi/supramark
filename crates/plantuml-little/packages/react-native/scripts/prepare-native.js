...
const MACOS_DYLIB_NAME = 'libsupramark_plantuml_native.dylib';
...
function prepareIOS() {
  ...
  copyDirRecursive(
    IOS_XCFRAMEWORK_SRC,
    path.join(IOS_FRAMEWORKS_DEST, 'SupramarkPlantuml.xcframework')
  );
  console.log(
    `✓ iOS: copied SupramarkPlantuml.xcframework → ${path.relative(REPO_ROOT, IOS_FRAMEWORKS_DEST)}`
  );
  return true;
}
...
function prepareAndroid() {
  ...
  if (!anyFound) {
    console.warn(
      `   Run \`cargo ndk -t arm64-v8a -t armeabi-v7a -t x86_64 -t x86 build --release -p supramark-plantuml-native\` first.`
    );
  }
  return anyFound;
}
